...
class ElixirConfig {
  ...
  modules(includes, excludes, fileName) {
    ...
    includes.forEach(baseDir => {
      ...
      let modules = fs
        ...
        .filter(file =>
          fs.statSync(path.resolve(this.prefix, baseDir, file)).isDirectory()
        )
        .filter(dir => excludes.indexOf(dir) < 0)
        .filter(dir =>
          fs.existsSync(path.join(this.prefix, baseDir, dir, fileName))
        );
  ...
  theme(location) {
    ...
    this.prefix = `${
      this.prefix
    }modules_app/contentbox-custom/_themes/${location}/`;
    const themeRecipe = require(path.resolve(
      global.elixir.rootPath,
      this.prefix,
      "elixir-theme"
    ));
    ...
    const name = this.prefix
      .split("/")
      .filter(s => s !== "")
      .filter(s => s !== "modules_app/contentbox-custom/_themes")
      .join("/");
    this.mergeConfig({
      resolve: {
        alias: {
  ...
  themes(includes, excludes, fileName) {
    includes = includes || ["modules_app/contentbox-custom/_themes"];
    excludes = excludes || [];
    fileName = fileName || "elixir-theme.js";
  ...
  installMissingDependencies() {
    if (this.missingDependencies.size !== 0) {
      ...
      console.log("Dependencies installed.  Please run ColdBox Elixir again.");
      process.exit(1);
    }
    return this;
  }
